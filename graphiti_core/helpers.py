...
# Lower values = more granular chunks = more edges per node
# Higher values = larger chunks = more context preserved
CHUNK_TOKEN_SIZE = int(os.getenv('CHUNK_TOKEN_SIZE', 1000))
...
# For JSON: elements per 1000 tokens > threshold * 1000 (e.g., 0.08 = 80 elements/1000 tokens)
# For Text: capitalized words per 1000 tokens > threshold * 500 (e.g., 0.08 = 40 caps/1000 tokens)
# Lower values = more aggressive chunking (good for scientific/technical content)
# Higher values = more conservative (good for narrative/prose)
CHUNK_DENSITY_THRESHOLD = float(os.getenv('CHUNK_DENSITY_THRESHOLD', 0.08))
